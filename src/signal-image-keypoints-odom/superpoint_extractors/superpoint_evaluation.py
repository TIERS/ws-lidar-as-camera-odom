...
class ImageProcessor:
  def __init__(self, weights_path, nms_dist, conf_thresh, nn_thresh, cuda):
    ...
    self.image_sub = rospy.Subscriber("/img_node/signal_image", Image, self.img_callback)
    self.pc_sub = rospy.Subscriber("/os_cloud_node/points", PointCloud2, self.pc_callback)
    self.points_pub = rospy.Publisher('/keypoint_point_cloud', PointCloud2, queue_size=10)
